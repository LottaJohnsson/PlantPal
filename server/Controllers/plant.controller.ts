...
router.get("/isAuthenticated", (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({message: "Internal server error: could not determine if user is authenticated"});
    }
});
...
router.post("/add", requireAuth, upload.single("imageFile"), async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.error("Internal server error: unable to add plant:", error);
        res.status(500).json({success: false, message: "Internal server error: unable to add plant:"});
    }
});
...
router.get("/get", requireAuth, async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({success: false, message: "Internal server error: unable to get user plants"});
    }
});
